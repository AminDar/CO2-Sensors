...
def split_data_frame(point):
    ...
    for start, end in zip([0] + slice_data, slice_data + [len(integral) - 1]):
        ...
        period = period.iloc[1:]  # Skip the first data point
...
def periodic_integral(point, df, segment_index):
    ...
    if segment_index % 2 != 0:
        ...
        ymin = y.max() if len(y) > 0 else 0  # For even segments, integrate with respect to the maximum concentration in the segment
