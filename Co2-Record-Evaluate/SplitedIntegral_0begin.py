...
def periodic_integral(point, df):
    ...
    # Zero adjustment for ymin
    ymin = 0
    x_min, x_max = df['time'].min(), df['time'].max()
    idx = np.where((np.array(x) >= x_min) & (np.array(x) <= x_max))[0]

    plt.yticks(np.arange(0, max(y) + 1, 150), fontsize=12)  # Set y-axis ticks starting from 0
...
for i in a:
    periodic_integral(eval_point, i)